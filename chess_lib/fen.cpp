...
bool FEN::applyFen(Board &board, const std::string_view fen) {
	...
	while (stream >> record) {
			...
			for (const char c : record) {
				if (c == '/') {
					...
					if (isdigit(c)) {
						...
						const ChessColor color = isupper(c) ? White : Black;
		...
		case FenRecord::ActiveColor:
			switch (record[0]) {
			case 'w':
				board.m_state.turnColor = White;
				break;
			case 'b':
				board.m_state.turnColor = Black;
				break;
			default:
				std::cerr << "Invalid active color in fen: " << record[0] << std::endl;
				return false;
			}
			break;
	...
	board.getPlayer(ChessColor::White).calculateMaterialValue(board.m_pieces);
	board.getPlayer(ChessColor::Black).calculateMaterialValue(board.m_pieces);
